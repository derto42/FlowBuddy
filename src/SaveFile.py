...
def get_setting(name: str) -> Union[any, None]:
    """Returns 'None' if no setting is defined."""
    return _settings[name] if name in _settings.keys() else None
...
def get_tasks(group_name: str) -> List[Task]:
    ...
    return [Task(group_name, task_name) for task_name in group]
